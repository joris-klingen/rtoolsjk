...
rk_barchart <- function(data, x, y, labels=F, sort=F, fill_color="#004699"){
  if (sort==T){
    print(data)
    data <- data %>%
  ...
  fig <- fig +
    rk_theme(orientation="horizontal", drop_axis_titles = T) +
    scale_y_continuous(
...
rk_format_number <- function(x){
  return(format(x, big.mark = ".", decimal.mark = ",", scientific = FALSE))
}

rk_stacked_bar_abs <- function(
    data,
    x,
    y,
    color_col,
  ...
  if (minimum_label_value >= 0){
    label <- ifelse(label < minimum_label_value, "", rk_format_number(round(label, 0)))
  }
  ...
  fig <- fig +
    ...
    scale_y_continuous(labels=function(x) rk_format_number(x))
...
rk_grouped_bar_abs <- function(
  ...
  # find better solution to avoid repetition in this block
